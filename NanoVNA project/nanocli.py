...
# configuration  (burada text yazımı için configürasyon ayarı yapılıyor)
...
#argparse kütüphanesi fonksiyonu gösteriliyor argparse argüment parçalayıcı anlamına gelir
def parse_args():
...
def save_data(freq, data, one_port, db_flag, name):

        file=open("/home/pi/progetto/nanovna_data/"+name,"w")
        file.write('# MHz S {} R 50'.format('DB' if db_flag else 'MA'))
        ...
        for f, d in zip(freq, data):
            ...
            file.write('\n'+'{:<10.6g}'.format(f/1e6))
            real_s11 = abs(d[0])
            com_s11 = np.angle(d[0], deg=True)
            s11 = np.sqrt(real_s11**2 + com_s11**2)
            s11_array.append(s11)
            vswr.append((1+abs(s11))/(1-abs(s11)))
        
            if one_port:
                file.write('{}'.format(one))
                
            else:
                file.write('{}{}{}{}'.format(one, two, two, one))
        
        reg_array = [1,1,1,1,1]
        prediction = sum(np.multiply(vswr, reg_array))
        print("vswr è ", vswr)
        print("il tipo di vswr è ", type(vswr))
        print("prediction è ", prediction)
        print("il tipo di prediction è", type(prediction))
        file.close()

def cycle(end, pause, cal):         
    
    n=0
    while True:
        # interpolate and measure
        ora=time.ctime()
        print("Start of acquisition, ", ora)
        ...
        # save data
        save_data(freq=freq, data=data, one_port=args.one_port, db_flag=args.db, name=ora)
        print("file ",n, " succesfully saved, ", time.ctime())
        n=n+1
        
        if time.time()>=end-pause:
            break
...
def main():
    ...
    # inserire l'inizio
    while True:
        print("\nEnter data and time of START in the format dd/mm/yyyy hh:mm\n(for example 19/05/2022 17:15)\n")
        inizio = time.strptime(input(),"%d/%m/%Y %H:%M")
        print("\nThe data entered is ",time.asctime(inizio),"\nTo confirm, answer \"y\", to change answer any other letter")
        risposta = input()
        
        if time.mktime(inizio) < time.time():
            print("\nERROR: The starting time can not be in the past.")
            continue
        if risposta == 'y':
                break
    ...
    # inserire la fine
    while True:
        print("\nEnter data and time of END in the format dd/mm/yyyy hh:mm\n(for example 19/05/2022 17:15)\n\n")
        fine = time.strptime(input(),"%d/%m/%Y %H:%M")
        print("\nThe data entered is ",time.asctime(fine),"\nTo confirm, answer  \"y\", to change answer any other letter ")
        risposta=input()
        if time.mktime(fine) < time.time() :
            print("\nError: the end time can not be in the past")
            continue
        elif time.mktime(fine) <= time.mktime(inizio):
            print("\nError: the end time can not come before or at the same time of the starting time.")
            continue
        if risposta == 'y':
                break
    
        
    # inserire il tempo di pausa
    while True:
        pause=int(input("Enter the time interval between two acquisitions (in seconds): "))
        if pause<0:
            print("ERROR: time can not be negative")
            continue
        break
    #attendi l'orario di avvio
    delay = time.mktime(inizio) - time.time()
    s.enter(delay, 1, cycle, (time.mktime(fine),pause,cal))
    s.run()
